...
public class ArtistDAOImpl extends UserDAOImpl implements ArtistDAO {
	...
	public List<FestivalEvent> getFestivalEventByArtistName(String name) {
		...
		for (EventRole e : roles)
			if (e.getUserRole().toLowerCase().contains("artist"))
	...
	private int getIdByName(String name) {
		Session sess = sf.openSession();
		String hql = "select id from artist where artist_name = :name";
